...
class DiffieHellman(object):
    def __init__(self, p=None, g=None, private_key=None):
        ...
        :Parameters:
            ...
            private_key : integer, long, or bytes
                the private key to use. Should be generated from a secure RNG.
                it should be in the range of p > private_key > 1
        """
        if p is not None and g is not None:
            self.p = p
            self.g = g
        else:
            print 'Using default prime and generator'
    ...
    def generateKeys(self):
        ...
        :Return:
            The generated public key based on g**private_key % p; send this to the other
            party.
        """
        if not self.private_key:
            self.private_key = randint(1, self.p - 1)
        
        private_key = self.private_key
        if isinstance(private_key, bytes):
            # Convert for our purposes
            private_key = bytes_to_long(self.private_key)

        self.public_key = pow(self.g, self.private_key, self.p)
        return long_to_bytes(self.public_key)


    def computeKey(self, rpublic_key):
        """Compute the shared secret

        :Parameters:
            rpublic_key : integer, long, or bytes
                remote public key
        :Return:
            The shared secret, derived from rpublic_key**private_key % p
        """
        if not self.private_key:
            raise DHError('Private key not generated')

        if isinstance(rpublic_key, bytes):
            rpublic_key = bytes_to_long(rpublic_key)

        return long_to_bytes(pow(rpublic_key, self.private_key, self.p))
